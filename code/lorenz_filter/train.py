...
def train_model(model_class, total_timesteps=1000000,add_noise=False,add_filter=False):
    # 这里直接在 make 时传参，简化代码

    env=DummyVecEnv([lambda:make_env(add_noise=add_noise,add_filter=add_filter)])
    ...
    if add_noise:
        model.save("hr_paper_noisy_model_filter") # 这会生成一个 .zip 文件
    else:
        model.save("hr_paper_final_model_filter") # 这会生成一个 .zip 文件
    print("训练成功！模型已保存为 zip")

if __name__ == "__main__":
    train_model(PPO, total_timesteps=500000, add_noise=False,add_filter=True)  # 训练无噪声版本
    train_model(PPO, total_timesteps=500000, add_noise=True,add_filter=True)  # 训练有噪声版本