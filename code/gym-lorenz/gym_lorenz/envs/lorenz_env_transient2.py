...
class lorenzEnv_transient(gym.Env):
    ...
    def step(self, action):
        ...
        dx_1_controlled_2 = self.a * (2 * state2[3] * state2[3] * (state2[1] - state2[0]) + self.d * state2[0])+ self.u1*100
        dx_2_controlled_2 = self.b * (2 * state2[3] * state2[3] * (state2[0] - state2[1]) - state2[2]) + self.u2*100
        dx_3_controlled_2 = self.c * (state2[1] - self.h * state2[2])
        dx_4_controlled_2 = state2[1] - state2[0] - 0.01 * state2[3]+self.u3*100
        ...
        now = self._get_observation()
        reward = -sum(abs(x) for x in now[0:4])-sum(abs(x) for x in now[0:4])**(1/3)  # 使用简单的欧氏距离作为奖励
        self.t = self.t + 0.001
        # if abs(self.u1-self.input_min)<10 or abs(self.u1-self.input_max)<10:
        #     reward=reward-1
        # if abs(self.u2-self.input_min)<10 or abs(self.u2-self.input_max)<10:
